...
核心思路: fakeip 模式下客户端在探测阶段只接触 FAKE-IP 探测项, 无法获得真实节点地址; direct/mixed 模式会按配置包含真实项
引导结束后, 客户端才拿到选中节点的真实 IP 建立业务连接

## 设计动机

- **隐藏真实节点** — fakeip 模式下探测流量与引导流量中不出现真实 IP, hacker 抓包或读日志无法直接锁定节点
...
| `mixed` | 同时下发真实项与有效 FAKE 项, 选路按全链路 RTT 最小选择 |
...
- **失效即时上报**: Valid 项变为 Invalid 时同样会触发 `fake_update`, Center 会立即移除失效项, 避免客户端继续探测坏项
- **共享 FAKE-IP 多候选**: 多个节点配置相同 FAKE-IP 时, 客户端只探测一次, 但该探测项会保留全部候选节点, 选路时分别计算全链路延迟
...
- 旧缓存文件 (探测项字段缺失) 自动合成真实项兜底, 保证 direct 行为不退化; 新模式明确返回空列表时不会回退合成真实项
